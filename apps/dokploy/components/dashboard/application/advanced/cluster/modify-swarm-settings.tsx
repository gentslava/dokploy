...
const addSwarmSettings = z.object({
  ...
  restartPolicySwarm: createStringToJSONSchema(RestartPolicySwarmSchema).nullable(),
  placementSwarm: createStringToJSONSchema(PlacementSwarmSchema).nullable(),
  updateConfigSwarm: createStringToJSONSchema(UpdateConfigSwarmSchema).nullable(),
  rollbackConfigSwarm: createStringToJSONSchema(UpdateConfigSwarmSchema).nullable(),
...
export const AddSwarmSettings = ({ applicationId }: Props) => {
  ...
  const { mutateAsync, isError, error, isLoading } = api.application.update.useMutation();
  ...
  useEffect(() => {
    if (data) {
      form.reset({
        healthCheckSwarm: data.healthCheckSwarm ? JSON.stringify(data.healthCheckSwarm, null, 2) : null,
        restartPolicySwarm: data.restartPolicySwarm ? JSON.stringify(data.restartPolicySwarm, null, 2) : null,
        placementSwarm: data.placementSwarm ? JSON.stringify(data.placementSwarm, null, 2) : null,
        updateConfigSwarm: data.updateConfigSwarm ? JSON.stringify(data.updateConfigSwarm, null, 2) : null,
        rollbackConfigSwarm: data.rollbackConfigSwarm ? JSON.stringify(data.rollbackConfigSwarm, null, 2) : null,
        modeSwarm: data.modeSwarm ? JSON.stringify(data.modeSwarm, null, 2) : null,
        labelsSwarm: data.labelsSwarm ? JSON.stringify(data.labelsSwarm, null, 2) : null,
        networkSwarm: data.networkSwarm ? JSON.stringify(data.networkSwarm, null, 2) : null,
      });
    }
    ...
    <Dialog>
      ...
      <DialogContent className='sm:max-w-5xl p-0'>
        <DialogHeader>
          ...
          <DialogDescription>{t('dashboard.swarm.description')}</DialogDescription>
        </DialogHeader>
        {isError && <AlertBlock type='error'>{error?.message}</AlertBlock>}
        <div className='px-4'>
          <AlertBlock type='info'>{t('dashboard.swarm.settingsWarning')}</AlertBlock>
        </div>
                ...
                <FormItem className='relative max-lg:px-4 lg:pl-6 '>
                  ...
                  <TooltipProvider delayDuration={0}>
                    <Tooltip>
                      ...
                      <TooltipContent className='w-full z-[999]' align='start' side='bottom'>
                        <code>
                          <pre>
                            {`{
                ...
                <FormItem className='relative  max-lg:px-4 lg:pr-6 '>
                  ...
                  <TooltipProvider delayDuration={0}>
                    <Tooltip>
                      ...
                      <TooltipContent className='w-full z-[999]' align='center' side='bottom'>
                        <code>
                          <pre>
                            {`{
                      ...
                      placeholder={`[
 {
	...
	"com.example.app.version" : "1.0.0"
            ...
            <DialogFooter className='flex w-full flex-row justify-end md:col-span-2 m-0 sticky bottom-0 right-0 bg-muted border'>
              <Button isLoading={isLoading} form='hook-form-add-permissions' type='submit'>
