...
export const ShowSecurity = ({ applicationId }: Props) => {
  ...
  const { mutateAsync: deleteSecurity, isLoading: isRemoving } = api.security.delete.useMutation();
  ...
  return (
    <Card className='bg-background'>
      <CardHeader className='flex flex-row justify-between flex-wrap gap-4'>
        <div>
          <CardTitle className='text-xl'>{t('dashboard.security.security')}</CardTitle>
          <CardDescription>{t('dashboard.security.description')}</CardDescription>
        </div>

        {data && data?.security.length > 0 && (
          <HandleSecurity applicationId={applicationId}>{t('dashboard.security.addSecurity')}</HandleSecurity>
        )}
      </CardHeader>
      <CardContent className='flex flex-col gap-4'>
        {data?.security.length === 0 ? (
          <div className='flex w-full flex-col items-center justify-center gap-3 pt-10'>
            ...
            <span className='text-base text-muted-foreground'>{t('dashboard.security.noSecurityConfigured')}</span>
            <HandleSecurity applicationId={applicationId}>{t('dashboard.security.addSecurity')}</HandleSecurity>
          </div>
        ) : (
                ...
                <div key={security.securityId}>
                  <div className='flex w-full flex-col md:flex-row justify-between md:items-center gap-4 md:gap-10 border rounded-lg p-4'>
                    ...
                    <div className='flex flex-row gap-2'>
                      <HandleSecurity securityId={security.securityId} applicationId={applicationId} />
                      <DialogAction
                        title={t('dashboard.security.deleteSecurity')}
                        description={t('dashboard.security.deleteSecurityConfirmation')}
                        type='destructive'
                        onClick={async () => {
                          await deleteSecurity({
                            ...
                            .then(() => {
                              ...
                              toast.success(t('dashboard.security.securityDeletedSuccessfully'));
                            })
                            .catch(() => {
                              toast.error(t('dashboard.security.errorDeletingSecurity'));
                            });
                        }}
                      >
                        <Button
