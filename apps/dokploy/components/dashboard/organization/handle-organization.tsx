  ...
  const onSubmit = async (values: OrganizationFormValues) => {
    await mutateAsync({
      ...
      .catch((error) => {
        console.error(error);
        toast.error(
          organizationId ? t('dashboard.organization.errorUpdating') : t('dashboard.organization.errorCreating')
        );
      });
  };
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        {organizationId ? (
          <DropdownMenuItem className='group cursor-pointer hover:bg-blue-500/10' onSelect={(e) => e.preventDefault()}>
          ...
          <DropdownMenuItem className='gap-2 p-2' onSelect={(e) => e.preventDefault()}>
            ...
            <div className='font-medium text-muted-foreground'>{t('dashboard.organization.createOrganization')}</div>
          </DropdownMenuItem>
        )}
      </DialogTrigger>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className='tems-center gap-4'>
                  <FormLabel className='text-right'>{t('dashboard.organization.name')}</FormLabel>
                  <FormControl>
                    <Input
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className='gap-4'>
                  <FormLabel className='text-right'>{t('dashboard.organization.logoUrl')}</FormLabel>
                  <FormControl>
                    <Input
                      placeholder={t('dashboard.organization.logoUrlPlaceholder')}
                      {...field}
                      value={field.value || ''}
                      className='col-span-3'
                    />
