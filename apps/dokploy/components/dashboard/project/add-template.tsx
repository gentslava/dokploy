...
export const AddTemplate = ({ projectId, baseUrl }: Props) => {
  ...
  const { mutateAsync, isLoading, error, isError } = api.compose.deployTemplate.useMutation();
        ...
        <DialogHeader className='sticky top-0 z-10 bg-background p-6 border-b'>
          <div className='flex flex-col space-y-6'>
            <div className='flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6'>
              <div>
                <DialogTitle>{t('dashboard.template.createFromTemplate')}</DialogTitle>
                <DialogDescription>{t('dashboard.template.createFromTemplateDescription')}</DialogDescription>
              </div>
              <div className='flex flex-col sm:flex-row items-start sm:items-center gap-4'>
                ...
                <Popover modal={true}>
                  <PopoverTrigger asChild>
                    <Button variant='outline' className={cn('w-full sm:w-[200px] justify-between !bg-input')}>
                      {isLoadingTags
                  ...
                  <PopoverContent className='p-0' align='start'>
                    <Command>
                      <CommandInput placeholder={t('dashboard.template.searchTag')} className='h-9' />
                      {isLoadingTags && (
                        <span className='py-6 text-center text-sm'>{t('dashboard.template.loadingTagsMessage')}</span>
                      )}
                      <CommandEmpty>{t('dashboard.template.noTagsFound')}</CommandEmpty>
                ...
                <Button
                  ...
                  className='h-9 w-9'
                >
                  {viewMode === 'detailed' ? <LayoutGrid className='size-4' /> : <List className='size-4' />}
                </Button>
              </div>
            </div>
            {selectedTags.length > 0 && (
              ...
              >
                {templates?.map((template) => (
                  ...
                  >
                    ...
                    >
                      <img
                        src={`${customBaseUrl || 'https://templates.dokploy.com/'}/blueprints/${template?.id}/${
                          template?.logo
                        }`}
                        className={cn('object-contain', viewMode === 'detailed' ? 'size-24' : 'size-16')}
                        alt={template?.name}
                      />
                      <div className='flex flex-col items-center gap-2'>
                        <span className='text-sm font-medium line-clamp-1'>{template?.name}</span>
                        {viewMode === 'detailed' && template?.tags?.length > 0 && (
                          <div className='flex flex-wrap justify-center gap-1.5'>
                            {template?.tags?.map((tag) => (
                              <Badge key={tag} variant='green' className='text-[10px] px-2 py-0'>
                                {tag}
                              </Badge>
                            ))}
                          </div>
                    ...
                    >
                      ...
                      <AlertDialog>
                        ...
                        <AlertDialogContent>
                          <AlertDialogHeader>
                            <AlertDialogTitle>{t('dashboard.template.areYouSure')}</AlertDialogTitle>
                            <AlertDialogDescription>
                                ...
                                <TooltipProvider delayDuration={0}>
                                  <Tooltip>
                                    ...
                                    <TooltipContent className='z-[999] w-[300px]' align='start' side='top'>
                                      <span>
                                        If no server is selected, the application will be deployed on the server where
                          ...
                          <AlertDialogFooter>
                            <AlertDialogCancel>{t('dashboard.template.cancel')}</AlertDialogCancel>
                            <AlertDialogAction
                              disabled={isLoading}
                              onClick={async () => {
                                ...
                                toast.promise(promise, {
                                  ...
                                  success: () => {
                                    ...
                                    setOpen(false);
                                    return t('dashboard.template.createdSuccessfully', { name: template.name });
                                  },
                                  error: () => {
                                    return t('dashboard.template.errorDeploying', { name: template.name });
                                  },
                                });
                              }}
                            >
