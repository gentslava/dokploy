...
import { z } from 'zod';
import { MariadbIcon, MongodbIcon, MysqlIcon, PostgresqlIcon, RedisIcon } from '@/components/icons/data-tools-icons';
...
export const AddDatabase = ({ projectId, projectName }: Props) => {
  ...
  const onSubmit = async (data: AddDatabase) => {
    const defaultDockerImage = data.dockerImage || dockerImageDefaultPlaceholder[data.type];
    ...
    if (data.type === 'postgres') {
      promise = postgresMutation.mutateAsync({
        ...
        databaseUser: data.databaseUser || databasesUserDefaultPlaceholder[data.type] || 'postgres',
        serverId: data.serverId,
      });
      ...
      promise = mongoMutation.mutateAsync({
        ...commonParams,
        databasePassword: data.databasePassword,
        databaseUser: data.databaseUser || databasesUserDefaultPlaceholder[data.type] || 'mongo',
        serverId: data.serverId,
      ...
      promise = mariadbMutation.mutateAsync({
        ...
        databaseUser: data.databaseUser || databasesUserDefaultPlaceholder[data.type] || 'mariadb',
        serverId: data.serverId,
      });
      ...
      promise = mysqlMutation.mutateAsync({
        ...
        databaseUser: data.databaseUser || databasesUserDefaultPlaceholder[data.type] || 'mysql',
        databaseRootPassword: data.databaseRootPassword,
        serverId: data.serverId,
      });
    }
  ...
  return (
    <Dialog open={visible} onOpenChange={setVisible}>
      ...
      <DialogContent className='md:max-h-[90vh]  sm:max-w-2xl'>
        <DialogHeader>
          <DialogTitle>{t('dashboard.project.addDatabase.dialogTitle')}</DialogTitle>
        </DialogHeader>

        <Form {...form}>
          <form id='hook-form' onSubmit={form.handleSubmit(onSubmit)} className='grid w-full gap-8 '>
            <FormField
                  ...
                  <FormControl>
                    ...
                    >
                      {Object.entries(databasesMap).map(([key, value]) => (
                        <FormItem key={key} className='flex w-full items-center space-x-3 space-y-0'>
                          <FormControl className='w-full'>
                            <div>
                              ...
                              >
                                {value.icon}
                                {t(`dashboard.project.addDatabase.databasesMap.${key}`)}
                              </Label>
                            </div>
                          </FormControl>
                        </FormItem>
            ...
            <div className='flex flex-col gap-4'>
              ...
              <div className='flex flex-col gap-2'>
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>{t('dashboard.project.addDatabase.name')}</FormLabel>
                      <FormControl>
                        <Input
                          placeholder={t('dashboard.project.addDatabase.namePlaceholder')}
                          {...field}
                          onChange={(e) => {
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder={t('dashboard.project.addDatabase.appNamePlaceholder')} {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>{t('dashboard.project.addDatabase.description')}</FormLabel>
                      <FormControl>
                        <Textarea
                          className='h-24'
                          placeholder={t('dashboard.project.addDatabase.descriptionPlaceholder')}
                          {...field}
                          value={field.value || ''}
                        />
                      </FormControl>
                ...
                />
                {(type === 'mysql' || type === 'mariadb' || type === 'postgres') && (
                  <FormField
                    control={form.control}
                    name='databaseName'
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>{t('dashboard.project.addDatabase.databaseName')}</FormLabel>
                        <FormControl>
                          <Input placeholder={t('dashboard.project.addDatabase.databaseNamePlaceholder')} {...field} />
                        </FormControl>
                ...
                {(type === 'mysql' || type === 'mariadb' || type === 'postgres' || type === 'mongo') && (
                  <FormField
                    control={form.control}
                    name='databaseUser'
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>{t('dashboard.project.addDatabase.databaseUser')}</FormLabel>
                        <FormControl>
                          <Input
                            placeholder={t('dashboard.project.addDatabase.databaseUserPlaceholder', {
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>{t('dashboard.project.addDatabase.databasePassword')}</FormLabel>
                      <FormControl>
                        <Input
                          type='password'
                          placeholder={t('dashboard.project.addDatabase.databasePasswordPlaceholder')}
                ...
                {(type === 'mysql' || type === 'mariadb') && (
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>{t('dashboard.project.addDatabase.databaseRootPassword')}</FormLabel>
                        <FormControl>
                          <Input
                            type='password'
                            placeholder={t('dashboard.project.addDatabase.databaseRootPasswordPlaceholder')}
                            {...field}
                          />
                        </FormControl>
                ...
                <FormField
                  ...
                  render={({ field }) => {
                    return (
                      <FormItem>
                        <FormLabel>{t('dashboard.project.addDatabase.dockerImage')}</FormLabel>
                        <FormControl>
                          <Input
                            placeholder={t('dashboard.project.addDatabase.dockerImagePlaceholder', {
                      ...
                      return (
                        <FormItem className='flex flex-row items-center justify-between p-3 mt-4 border rounded-lg shadow-sm'>
                          <div className='space-y-0.5'>
                            <FormLabel>{t('dashboard.project.addDatabase.useReplicaSets')}</FormLabel>
                          </div>
                          <FormControl>
          ...
          <DialogFooter>
            <Button isLoading={form.formState.isSubmitting} form='hook-form' type='submit'>
